...
int main() {
    ...
    for(int i=0; i<=mk1; i++){ 
        for(int j=0; j<=mk2; j++){
            if(i-b1 >= 0 && !dp[i-b1][j]){dp[i][j]=1; continue;} // [i-1][j]
            if(j-b1 >= 0 && !dp[i][j-b1]){dp[i][j]=1; continue;} // [i][j-1]
            if(i-b2 >= 0 && !dp[i-b2][j]){dp[i][j]=1; continue;} // [i-3][j]
            if(j-b2 >= 0 && !dp[i][j-b2]){dp[i][j]=1; continue;} // [i][j-3]
            if(i-b3 >= 0 && !dp[i-b3][j]){dp[i][j]=1; continue;} // [i-4][j]
            if(j-b3 >= 0 && !dp[i][j-b3]){dp[i][j]=1; continue;} // [i][j-4]
        }
    }
